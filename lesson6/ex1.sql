...
WHERE `to_users_id` = 101 
	AND `from_users_id` IN (
		...
		WHERE (`to_users_id` = 101) AND STATUS = 1)
	AND (SELECT COUNT(*) AS `count` FROM `my_vk_schema`.`messages` WHERE `to_users_id` = 101 GROUP BY `from_users_id` ORDER BY `count` DESC LIMIT 1)
